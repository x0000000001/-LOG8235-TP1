...
bool PhysicsHelpers::SphereCast(const FVector& start, const FVector& end, float radius, TArray<struct FHitResult>& outHits, bool drawDebug)
{
    ...
    FCollisionObjectQueryParams objectQueryParams;
    objectQueryParams.IgnoreMask = ECC_WorldStatic;
...
bool PhysicsHelpers::SphereOverlap(const FVector& pos, float radius, TArray<struct FOverlapResult>& outOverlaps, bool drawDebug)
{
    ...
    // Set up query parameters to include only dynamic objects
    FCollisionObjectQueryParams objectQueryParams;
...
TArray<FHitResult> PhysicsHelpers::FilterHitResultsByTraceChannel(const TArray<FHitResult>& InHitResults, const ECollisionChannel filteredChannel)
{
    TArray<FHitResult> FilteredHitResults;

    // Use TArray::FilterByPredicate to filter the hit results based on the trace channel
    FilteredHitResults = InHitResults.FilterByPredicate([filteredChannel](const FHitResult& HitResult)
        {
            ...
            if (HitComponent)
            {
                ...
                // Compare the collision channel with ECC_GameTraceChannel3
                return CollisionChannel == filteredChannel;
            }

            // If the hit component is null, exclude it from the filtered results
            return false;
        });
